...
def compute_scores(gndPath, predPath,data_path):
    TN = 0
    TP = 0
    FN = 0
    FP = 0
    ...
    for i in range(len(gnd_ids)):
        ...
        pre = cv2.resize(pre, (gnd.shape[1], gnd.shape[0]), cv2.INTER_NEAREST)
        ...
        # print(((pre < 255) & (pre > 0)).sum())
        # print(((gnd < 255) & (gnd > 0)).sum())
        
        predPath_ = os.path.join("./pred/",data_path, pred_ids[i])
 
        # gndPath_ = os.path.join("./pred/tail/1gnd", gnd_ids[i])
        # gndPath_ += ".png"
        # predPath_ += ".png"
        #print(type(gnd[0][0]))
        #print(predPath_)
        ...
        cv2.imwrite(predPath_, pre)
        # cv2.imwrite(gndPath_, gnd)
        # gnd = cv2.imread('/media/preethamam/Utilities/Xtreme_Programming/Liu/project-dlcrack-2d/pred/tail/1gnd/11125-1.jpg')
        # print(((gnd < 255) & (gnd > 0)).sum())
        # break
...
if __name__ == "__main__":
    ...
    predPath = "./DS3"
    gndPath = "/media/preethamam/Utilities/Xtreme_Programming/Z_Data/DLCrack/Liu+Xincong+DS3+CrackSegNet/Testing/"+"DS3"+"/TestingCracksGroundtruth"
    TN, FP, FN, TP = compute_scores(gndPath, predPath,args.data)
