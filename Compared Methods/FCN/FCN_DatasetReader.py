...
class DatasetReader():
    ...
    def _read_images(self, image_files):
        return np.array([self._read_image(img_file) if path.exists(img_file) else print("MISSING:", img_file) for img_file in image_files])

    def _read_image(self, image_file):
        image = imageio.imread(image_file)
        if self.resize:
            resize_image = np.array(Image.fromarray(image).resize((self.width, self.height), Image.NEAREST)).astype(np.double)
        else:
            resize_image = image
        ...
        # check fate jpg - rgb
        if image_file[-3:] == 'jpg':
            if resize_image.shape == (224, 224, 1):
                resize_image = np.stack((resize_image,)*3, axis=3).reshape(224, 224, 3)
            assert resize_image.shape == (224, 224, 3), print(image_file, resize_image.shape)

        # check fate jpg - gray
        if image_file[-3:] == 'png' and "Groundtruth" in image_file:
            if resize_image.shape == (224, 224, 3):
                resize_image = resize_image[:,:,0].reshape(224, 224, 1)
            assert resize_image.shape == (224, 224, 1), print(image_file, resize_image.shape)
...
class ImageReader():
    def __init__(self, image_dir):
        self.img_files = glob.glob(path.join(image_dir, '*.png'))
