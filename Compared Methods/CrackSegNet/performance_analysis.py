...
def compute_scores(gndPath, predPath):
    ...
    for i in range(len(gnd_ids)):
        ...
        pre = cv2.resize(pre, (gnd.shape[1], gnd.shape[0]), cv2.INTER_LINEAR)
        #gnd = cv2.resize(gnd, (gnd.shape[1], gnd.shape[0]), cv2.INTER_LINEAR)
        print('gnd shape',gnd.shape)
        ...
        predPath_ = os.path.join("./ExperimentResults/DS3_resize/", pred_ids[i])
...
if __name__ == "__main__":
    ...
    predPath = "./ExperimentResults/DS3"
    gndPath = "./Testing/DS3/TestingCracksGroundtruth"
    TN, FP, FN, TP = compute_scores(gndPath, predPath)
    print("\n---Performance Scores---")
    get_scores(TN, FP, FN, TP)
